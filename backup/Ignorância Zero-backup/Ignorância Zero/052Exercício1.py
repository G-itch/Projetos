...
def Salvar(player_vida, player_sp, inimigos):
    salvar = open("arquivos/052Exercício1salvar.txt","w")
    salvar.write(str(player_vida)+"\n")
    salvar.write(str(player_sp)+"\n")
    salvar.write(str(inimigos)+"\n")


def CarregaJogo():
    carregar = open("arquivos/052Exercício1Salvar.txt","r")
    player_vida = int(carregar.readline())
    player_sp = int(carregar.readline())
    inimigos = eval(carregar.readline())
