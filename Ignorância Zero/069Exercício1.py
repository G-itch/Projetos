...
"""
class sim_ou_não(Exception):
    def __str__(self):
        return ("\033[31mApenas digite sim ou não")  
class apenas_letras(Exception):
    def __str__(self):
        return (f"\033[31mDigite apenas letras")
class Algo_deu_errado(Exception):
    def __str__(self):
        return (f"\033[31mAlgo deu errado! Digite novamente.")
...
class Jogo(object):
    ...
    def apresentação(self):
        print("\033[94mPense em um número de 1 a 63 e eu adivinharei!")

    def recebeEntradaDoUsuário(self):
        while True:
            try:
                list_resp = [["sim","s"],"não","n"]
                x = input("\033[32mO seu número está aqui? ")
                if x.lower() in list_resp or x.lower() in list_resp[0]:
                    if x.lower() in list_resp[0]:
                        return True
                    else:
                        return False
                elif not x.isalpha():
                    raise apenas_letras
                elif not x.lower() in list_resp or x.lower() in list_resp[0]:
                    raise sim_ou_não
                else:
                    raise Algo_deu_errado
            except apenas_letras:
                print(apenas_letras())
            except sim_ou_não:
                print(sim_ou_não())
            except Algo_deu_errado:
                print(Algo_deu_errado())
            
        

    def imprimeNumeroSecreto(self):
        print(f"O seu número é {self.__num}")
    ...
    def mostraCartão(self):
        ...
        """
        print(f"\033[36m{self.__cartoes[self.__card]}")
